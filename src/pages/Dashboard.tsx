...
const Dashboard = () => {
  ...
  const fetchData = async () => {
    ...
    const { data: profileData } = await supabase
      .from('profiles')
      .select('balance, full_name, referral_code, is_active')
  ...
  return (
    <div className="space-y-4 md:space-y-6">
      ...
      {profile?.is_active ? (
        ...
        <div className="rounded-xl bg-gradient-to-r from-amber-500/10 to-orange-500/10 border border-amber-500/20 p-4 md:p-6">
          <div className="flex flex-col sm:flex-row sm:items-center gap-4">
            <div className="flex items-center gap-4 flex-1">
              ...
              <div className="min-w-0 flex-1">
                <h3 className="text-white font-semibold text-sm md:text-base">Ative sua conta para indicar</h3>
                <p className="text-xs text-gray-400">
                  Para compartilhar seu link de indicação e começar a ganhar comissões, ative pelo menos um robô.
                </p>
              </div>
            </div>
            <Link
